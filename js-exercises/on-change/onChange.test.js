...
describe('onChange', () => {
  ...
  test('throw type error incase wrong params are passed', () => {
    expect(() => onChange()).toThrow(TypeError);
    expect(() => onChange(new Map(), () => { })).toThrow(TypeError);
    expect(() => onChange(new Set(), () => { })).toThrow(TypeError);
    expect(() => onChange({}, {})).toThrow(TypeError);
  });
});
